...
def test_arguments():
    """Test arguments for arguments of test_aide.equality_helpers.assert_dict_equal_msg."""

    expected_arguments = ["actual", "expected", "msg_tag"]

    arg_spec = inspect.getfullargspec(eh.assert_dict_equal_msg)
...
def test_expected_not_dict_error():
    ...
    with pytest.raises(
        ...
        eh.assert_dict_equal_msg(expected=1, actual={}, msg_tag="test_msg")
...
def test_actual_not_dict_error():
    ...
    with pytest.raises(
        ...
        eh.assert_dict_equal_msg(expected={}, actual=1, msg_tag="test_msg")
    ...
    with mock.patch(
        ...
        for i, k in enumerate(actual_value.keys()):
            ...
            for i, (e, a) in enumerate(zip(call_n_pos_args, expected_pos_args)):
                ...
                assert (
                    e == a
                ), f"Difference in positional args call {i} to test_aide.equality_helpers.assert_equal_dispatch (for key {k}) -\n Expected: {e}\n  Actual: {a}"
